...
const SYSTEM_PROMPT = `당신은 한국 입시(수능/내신) 영어 독해 전문 강사를 돕는 분석 도우미입니다.
영어 독해 지문을 받으면 아래 작업을 수행합니다. 모든 설명은 한국어로 작성합니다.

1. 출처 추정: 수능/평가원 모의평가/교육청 학력평가/EBS 연계교재/교과서/원문(기사·서적·논문) 중 어디에서 온 지문인지 추정합니다.
   웹 검색 도구가 있으면 지문의 특징적인 구절을 검색해 원문과 기출 여부를 확인하세요.
   확실하지 않으면 confidence를 낮추고 근거를 솔직하게 밝히세요. 절대 출처를 지어내지 마세요.
2. 지문 분석: 주제, 요지, 문단(또는 의미 단위)별 전개 구조, 난이도, 전문 해석.
3. 중요 어휘: 시험에 나올 만한 핵심 단어 8~15개를 골라 단어별로 유의어 클러스터, 반의어, 자주 쓰이는 콜로케이션, 지문 속 쓰임을 정리합니다.
   유의어는 뉘앙스가 가까운 순서로, 콜로케이션은 실제로 빈출되는 조합만 넣으세요.
4. 수업 포인트: 수업에서 짚어야 할 구문(문법), 배경지식, 학생들이 헷갈릴 부분.
5. 문제 풀이 요령: 이 지문으로 출제될 만한 유형(주제/제목/요지/빈칸/순서/삽입/어법/어휘 등)을 골라 유형별 접근법을 지문에 맞춰 구체적으로 설명합니다.
...
/**
 * 지문을 분석한다. 텍스트 델타가 올 때마다 onDelta(text)를 호출하고,
 * 완료되면 { rawText, result } 를 반환한다. result는 파싱 실패 시 null.
 */
export async function analyzePassage({ apiKey, model, passage, useWebSearch, onDelta, onStatus }) {
  ...
  onStatus?.(useWebSearch ? "출처 검색과 분석을 시작합니다…" : "분석을 시작합니다…");
  ...
  const stream = client.messages.stream({
    ...
    messages: [
      {
        role: "user",
        content: `다음 영어 독해 지문을 분석해 주세요.${useWebSearch ? " 웹 검색으로 출처를 확인해 주세요." : " (웹 검색 없이 알고 있는 지식으로만 출처를 추정하세요.)"}\n\n<passage>\n${passage}\n</passage>`,
      },
    ],
  });
